...
contract('VacationFactory', async (accounts) => {
  ...
  it('should be able to create a Loner contract', async() => {
    let entryFee = 1e18;
    let deadline = Math.floor(new Date("12/15/2017") /1000);
    let dateBegin = Math.floor(new Date("12/23/2017") /1000);
    let dateEnd = Math.floor(new Date("12/28/2017") /1000);
    maxParticipants = 2;
    let whitelist = [];
    address = await vacationFactory.createLoner.call(entryFee, deadline, dateBegin, dateEnd, whitelist, maxParticipants, { from: accounts[0] });
    await vacationFactory.createLoner(entryFee, deadline, dateBegin, dateEnd, whitelist, maxParticipants, { from: accounts[0] });
    let vacation = Vacation.at(address);
    let priceInWei = await vacation.priceInWei();
    assert.equal(+priceInWei, 0);

    let vacationLength = await vacationFactory.getLonerLength();
    assert.equal(+vacationLength, 1);
    let vAddress = await vacationFactory.getLonerByIndex(0);
    assert.equal(vAddress, address);
  });
  it('should be able to create a DateNight contract', async() => {
    let entryFee = 1e18;
    let deadline = Math.floor(new Date("12/15/2017") /1000);
    let dateBegin = Math.floor(new Date("12/23/2017") /1000);
    let dateEnd = Math.floor(new Date("12/28/2017") /1000);
    maxParticipants = 2;
    let whitelist = [];
    address = await vacationFactory.createDateNight.call(entryFee, deadline, dateBegin, dateEnd, whitelist, maxParticipants, { from: accounts[0] });
    await vacationFactory.createDateNight(entryFee, deadline, dateBegin, dateEnd, whitelist, maxParticipants, { from: accounts[0] });
    let vacation = Vacation.at(address);
    let priceInWei = await vacation.priceInWei();
    assert.equal(+priceInWei, 0);

    let vacationLength = await vacationFactory.getDateNightLength();
    assert.equal(+vacationLength, 1);
    let vAddress = await vacationFactory.getDateNightByIndex(0);
    assert.equal(vAddress, address);
  });
});
